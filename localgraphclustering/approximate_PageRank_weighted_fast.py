...
class Approximate_PageRank_weighted_fast(GraphBase[Input, Output]):
    ...
    def produce(self,
        ...
        for input in inputs:
            ...
            (actual_length,actual_xids,actual_values) = aclpagerank_weighted_cpp(n,np.uint32(input.adjacency_matrix.indptr),
                np.uint32(input.adjacency_matrix.indices),np.uint32(input.adjacency_matrix.data),alpha,rho,
                [ref_node],1,iterations,xlength=xlength)
