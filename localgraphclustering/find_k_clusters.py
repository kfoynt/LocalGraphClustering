...
def graph_segmentation(g, 
    ...
    while True:
        ...
        ref_node = random.sample(candidates, 1)
        
        if not is_weighted:
            output = compute_embedding(g_copy,ref_node,rho_list,nsamples_from_rho,localmethod,alpha,normalize,normalized_objective,epsilon,iterations,is_weighted)
        else:
            rho = rho_list[0]
        ...
        candidates = candidates - set(output[0])
...
def compute_k_clusters_from_region_adjacency(g,nclusters,Z,info,linkage: str = 'complete'):
    """
