...
from localgraphclustering.sweep_general import sweep_general

Input = TypeVar('Input', bound=Graph)
Output = TypeVar('Output',bound=np.ndarray)


class SweepCut_general(GraphBase[Input, Output]):
    ...
    def produce(self, 
        ...
        return [sweep_general(p,input) for input in inputs]
